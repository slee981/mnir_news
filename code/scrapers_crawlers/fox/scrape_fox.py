...
# Constants
########################################
BASE_FOLDER          = '/home/stephen/Dropbox/CodeWorkspace/python/news-article-scraper/fox/'
...
def get_clean_txt(article_contents):
    ...
    for q in blockquote_txt: 
        text = text.replace(q.text, '')
    ...
    # TODO: remove the word Fox and FOX from the text
...
def write_txt_file(text, url, file_id, folder):
    ...
    # folder is formatted "/home/stephen/.../fox-topic"
    topic = folder[len(BASE_FOLDER)+4:]

    file_name = f"fox_{topic}_{file_id}.txt"
    with open(file_name, 'a') as f:
        f.write(f'<article id={file_name} url="{url}">\n' + text + '\n</article>')
